(async function(){
  ...
  function visibleBoards(){
    ...
    return Store.list('boards').filter(b => b.visibility === 'all' || (user && user.isAdmin) || (Array.isArray(b.visibility) && user && b.visibility.includes(user.id)));
  }
  ...
  function renderRail(){
    ...
    rail.querySelectorAll('.board-pill').forEach(p => p.addEventListener('click', () => { activeBoardId = p.dataset.id; renderAll(); }));
  }
  ...
  function renderBoardMain(){
    ...
    const groups = Store.list('boardGroups').filter(g=>g.boardId===board.id).sort((a,b)=>(a.order||0)-(b.order||0));
    main.innerHTML = `
  ...
  function groupHtml(g){
    const tasks = Store.list('tasks').filter(t=>t.groupId===g.id).sort((a,b)=>(a.order||0)-(b.order||0));
    return `
      <div class="board-group" data-id="${g.id}">
        <div class="board-group-head">
          ...
          <span class="g-count">${tasks.length} item${tasks.length===1?'':'s'}</span>
          ${g.automation ? `<span title="Automação: quando status = ${g.automation.onStatus}, move para outro quadro/grupo">⚡</span>` : ''}
          <button class="icon-btn automation-btn" data-id="${g.id}" style="width:28px;height:28px;" title="Configurar automação">⚙️</button>
          <button class="icon-btn delete-group-btn" data-id="${g.id}" style="width:28px;height:28px;" title="Excluir grupo">✕</button>
        </div>
  ...
  function taskRowHtml(t){
    ...
    return `
      <div class="board-item-row" data-id="${t.id}">
        ...
        <span class="item-name task-open-btn" data-id="${t.id}">${Util.escapeHtml(t.name)}</span>
        <select class="status-pill col-extra" data-id="${t.id}" style="background:${sc.bg};color:${sc.color};border:none;">
          ${STATUS_OPTIONS.map(s=>`<option value="${s}" ${t.status===s?'selected':''}>${s}</option>`).join('')}
        </select>
        <span class="col-extra text-faint text-xs" style="font-family:'Space Grotesk',sans-serif;">${Util.relativeDay(t.date)}</span>
        <span class="col-extra" title="${owner?owner.name:''}">${owner?Shell.userAvatarHtml(owner,'xs'):''}</span>
        <button class="icon-btn delete-task-btn" data-id="${t.id}" style="width:26px;height:26px;">✕</button>
      </div>`;
  }
  ...
  function wireGroupEvents(g){
    ...
    main.querySelector(`.delete-group-btn[data-id="${g.id}"]`)?.addEventListener('click', () => {
      Util.confirmModal(`Excluir o grupo "${g.name}" e todos os itens dentro dele?`, async () => {
        ...
        renderAll();
      }, { danger:true, okLabel:'Excluir grupo' });
    });
    ...
    main.querySelectorAll(`.group-items[data-group-id="${g.id}"] .delete-task-btn`).forEach(el => el.addEventListener('click', async () => {
      await Store.remove('tasks', el.dataset.id);
      renderBoardMain();
    }));
    main.querySelector(`.add-task-btn[data-group-id="${g.id}"]`)?.addEventListener('click', (e) => {
      ...
      async function commit(){
        ...
        if(name){
          ...
          await Store.insert('tasks', { boardId:g.boardId, groupId:g.id, name, status:STATUS_OPTIONS[0], owner:(currentUser()||{}).id, date:Store.isoDate(0), priority:'Média', order:count });
        }
        renderBoardMain();
      }
      input.addEventListener('blur', commit);
  ...
  function openBoardForm(board){
    ...
    if(isEdit) modal.querySelector('#deleteBoardBtn').addEventListener('click', () => {
      Util.confirmModal(`Excluir o quadro "${b.name}" e tudo dentro dele?`, async () => {
        ...
        Util.closeModal();
        renderAll();
      }, { danger:true, okLabel:'Excluir quadro' });
    });
  ...
  async function duplicateBoard(board){
    ...
    await Promise.all(tasks.map(t => Store.insert('tasks', { boardId:newBoard.id, groupId:groupIdMap[t.groupId], name:t.name, status:t.status, owner:t.owner, date:t.date, priority:t.priority, order:t.order, notes:t.notes })));
    activeBoardId = newBoard.id;
    renderAll();
  ...
  function openTaskDrawer(task){
    ...
    const drawer = Util.openDrawer(`
      ...
      <div class="modal-body" style="flex:1;overflow-y:auto;">
        ...
        <div class="field"><label>Notas</label><textarea id="tNotes" placeholder="Adicione o que quiser...">${Util.escapeHtml(task.notes||'')}</textarea></div>
        <div class="field-row">
    ...
    drawer.querySelector('#deleteTaskDrawerBtn').addEventListener('click', () => {
      Util.confirmModal('Excluir este item?', async () => {
  ...
  function renderChat(){
    ...
    const msgs = Store.list('boardChats').filter(c=>c.boardId===board.id).sort((a,b)=>a.createdAt.localeCompare(b.createdAt));
    chat.innerHTML = `
      <div class="board-chat-head">💬 Chat — ${Util.escapeHtml(board.name)}</div>
      <div class="board-chat-body" id="chatBody">
        ...
        }).join('') || '<p class="text-faint text-sm" style="padding:10px;">Nenhuma mensagem ainda.</p>'}
      </div>
      <div class="board-chat-input">
        <input type="text" id="chatInput" placeholder="Escrever mensagem...">
    ...
    async function send(){
      ...
      if(!text) return;
      input.value = '';
      await Store.insert('boardChats', { boardId:board.id, authorId:(currentUser()||{}).id, text, createdAt:new Date().toISOString() });
      renderChat();
    }
